...
```{r weighted mean elevations total cover}
...
####Can't get the apply function to work...so just have to cycle through all 11 manually for now....###


wtd.t.test(total_dist_dat_split1969OTV[[1]]$ELEV,total_dist_dat_split2015OTV[[1]]$ELEV, weight=as.numeric(total_dist_dat_split1969OTV[[1]]$TotalOTV), weighty=as.numeric(total_dist_dat_split2015OTV[[1]]$TotalOTV))
...
```{r ordination with OTV data}



totalOTV_wide <- spread(totalcover_longOTV2, SPECIES, TotalOTV)

##totalOTV_wide is the sp matrix##

##remove plot B61 b/c of no trees##

totalOTV_1<-subset(totalOTV_wide, PLOTYEAR!="B611969")
totalOTV_final<-subset(totalOTV_1, PLOTYEAR!="B612015")


####
siteandvegdata<-merge(totalOTV_final, env_imp, by="PLOTYEAR")
...
##
spp.relOTV <-decostand(totalOTV_final[2:13], method = "total")


##run Multiple Response Permutations Procedure (MRPP)
mrpp((spp.relOTV), sitedata$Year, permutations = 999, distance = "bray", weight.type = 1, strata = NULL, parallel = getOption("mc.cores"))

##run ordination
ordOTV <- metaMDS(spp.relOTV, k=3, autotransform = FALSE)
...
#environmental correaltion with axes 
env_fitOTV <- envfit(ordOTV, env_elev_hli, permu = 999)
env_fitOTV
...
#par(mfrow=c(2,1))
cols<-c("gray30", "green4")
plot(ordOTV, type="n", cex.lab=2)
plot(env_fitOTV, p.max=0.01, col= "black")

text(ordOTV, display = "spec", cex=.85, col="black", pch=21)
#dev.off()
...
plot(ordOTV, type="n", cex.lab=1.5,cex.axis=1.5, xlab="NMDS 1", ylab="NMDS 2")
box(lwd=2)
with(sitedata, points(ordOTV, display="sites", col=cols[Year], pch=21, bg=cols[Year]))
...
#jpeg("sawyer species turnover.jpg", units="in", width = 9, height = 5, res=350)####plot species with env

par(mfrow=c(1,2))
####
plot.decay(elev_decay.exp_2015, col="white", ylim=c(0,.75), main="Elevation")
...
legend(750, 0.7, legend=c("1969", "2015"),lty=1:1,
       col=c("gray30","green3"), lwd=2, box.lty=0)

##HLI PLOT
plot.decay(hli_decay.exp_2015, col="white", ylim=c(0,.75), main="Heat Load Index")
...
legend(0.45, 0.7, legend=c("1969", "2015"),lty=1:1,
       col=c("gray30","green3"), lwd=2, box.lty=0)
