...
ordinationplot <- ggplot() +xlim(-1.75, 1.9)+ylim(-1.75, 1.5) +stat_ellipse(geom="polygon",alpha = 0.2, data=NMDSandplots, aes(x = NMDS1, y = NMDS2, fill=Year))+ geom_point(data=NMDSandplots, aes(NMDS1, NMDS2, fill=Year), shape = 21, size=0.75)+mytheme+scale_fill_manual(values=alpha(c("gray30", "green4"), 1)) +geom_segment(data = env_fitdf , aes(x = 0, xend = NMDS1+1, y = 0, yend = NMDS2+1),     arrow = arrow(length = unit(0.25, "cm")), colour = "black") +annotate(geom="text", x=.75, y=1.2, label="HLI",  color="black")+annotate(geom="text", x=1.1, y=1.35, label="ELEV",  color="black")+ theme(legend.position = "right")
